...
function getJobList (){
  ...
  # Extract the job IDs
  JOBLIST=$(echo "${JOBLISTDATA}" | ggrep -o -P '(?<=searchRes"&gt;).*(?=&lt;/div&gt;&lt;div class="searchResCount)')
  ...
  JOBSREMAINING=$(echo "${JOBLISTDATA}" | ggrep -o -P '(?<=searchResRemaining"&gt;).*(?=&lt;/div&gt;&lt;)')
...
function getJobDescriptions (){
  ...
  do
    ...
    TITLE=$(echo "${JOBDATA}" | ggrep -o '<h3>.*</h3>' | sed 's/\(<h3>\|<\/h3>\)//g' | tail -n1)
    TITLEWITHDIVIDER="  |  ${TITLE}"
    COMPANY=$(echo "${JOBDATA}" | ggrep -A1 '<label>Employment' | tail -n1 | sed "s/\&amp;/\&/g" | tr -cd "tr -cd '[[:alnum:]'" | sed -e 's/^[ \t]*//')
      if [ -z "${COMPANY}" ]; then COMPANY=$(echo "$JOBDATA" | ggrep -A1 '<label>Company</label>' | tail -n1 | sed "s/\&amp;/\&/g" | tr -cd "tr -cd '[[:alnum:]'" | sed -e 's/^[ \t]*//'); fi
    COMPANYWITHDIVIDER="  |  ${COMPANY}"
    CONTACT=$(echo "${JOBDATA}" | ggrep -A1 'Job_Contact' | tail -n1 | tr -cd "tr -cd '[[:alnum:]'" | sed -e 's/^[ \t]*//')
      if [ -z "${CONTACT}" ]; then CONTACT="Name Not Supplied"; fi

    # Supress un-unique contacts
    if [ "${UN}" ]; then
      if ! ggrep -q "'${CONTACT}'" <<< "${ALLCONTACTS}" ; then
    ...
    if [ -z "${LI_FILE}" ]; then
      ...
      CHECK2=$(echo "${READ_LI_FILE}" | ggrep -i ${CONTACT_FIRSTNAME} | ggrep -i ${CONTACT_SURNAME} | ggrep -i ${COMPANY_FIRSTNAME} | wc -l )
